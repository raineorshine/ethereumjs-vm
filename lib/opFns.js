...
// gets the debug line number (assuming it was the last thing added to memory)
function getDebugLine (runState) {
  const lineData = runState.memory.slice(runState.memory.length-2)
  const lineNumber = lineData[0] * 256 + lineData[1]
  return lineNumber
}
...
function describeLocation (runState) {
  ...
  return 'transaction #' + hash
    + ', Contract: ' + address
    + ', Operation: ' + pc
    + (runState.debugLines ? ', Line: ' + getDebugLine(runState) : '')
}
